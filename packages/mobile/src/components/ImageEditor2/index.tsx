...
import { getPercentFromNumber, getPercentDiffNumberFromNumber } from './helpers'
...
class ImageCropper extends PureComponent<IProps, IState> {
  static crop = (params: ICropParams) => {
    const { cropSize, positionX, positionY, cropAreaSize, srcSize, fittedSize, scale } = params
  ...
  init = () => {
    const { width, height } = this.props.imageUri
  ...
  render() {
    ...
    const { imageUri, cropAreaWidth, cropAreaHeight, containerColor, areaColor } = this.props
