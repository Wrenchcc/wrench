    ...
    this.translateMaster = block([
      cond(
        ...
        [
          ...
          cond(eq(this.panMasterState, State.BEGAN), stopClock(this.masterClockForOverscroll)),
        ]
      ),
      cond(
  ...
  withEnhancedLimits(val, masterOffseted) {
    ...
    return block([
      ...
      cond(
        ...
        [
          cond(eq(this.panState, State.ACTIVE), set(masterOffseted, sub(masterOffseted, diffPres))),
          cond(greaterThan(masterOffseted, 0), [set(limitedVal, 0)]),
          cond(not(eq(this.panState, State.END)), set(justEndedIfEnded, 1)),
          cond(
            or(eq(this.panState, State.ACTIVE), eq(this.panMasterState, State.ACTIVE)),
