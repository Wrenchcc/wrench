...
import { ImageURISource, Platform, View, Dimensions } from 'react-native'
import {
  PanGestureHandler,
  PinchGestureHandler,
  State,
...
export default class ImageEditor extends React.PureComponent<PickerProps> {
  ...
  constructor(props: PickerProps) {
    ...
    this.distanceFromTop = divide(
      sub(this.componentHeight, min(this.componentHeight, divide(this.componentWidth, this.ratio))),
      2
    )

    this.scale = ImageEditor.withBouncyLimits(
      ImageEditor.withPreservingMultiplicativeOffset(
        this.scaleMovement,
        this.pinchState,
        this.props.minZoom,
        this.props.maxZoom
    ...
    this.transX = ImageEditor.withBouncyLimits(
      ImageEditor.withDecaying(
        ImageEditor.withAddingFocalDisplacement(
          ImageEditor.withPreservingAdditiveOffset(this.dragX, this.panState, this.scale),
    ...
    this.transY = ImageEditor.withBouncyLimits(
      ImageEditor.withDecaying(
        ImageEditor.withAddingFocalDisplacement(
          ImageEditor.withPreservingAdditiveOffset(this.dragY, this.panState, this.scale),
    ...
    return block([
      cond(
        or(
          ...
          anotherState ? and(eq(state, State.END), eq(anotherState, State.ACTIVE)) : 0
        ),
          ...
          cond(
            ...
            [
              cond(
                ...
                set(
                  limitedVal,
                  ImageEditor.runSpring(springClock, limitedVal, minBound, flagWasRunSpring)
                ),
                cond(
                  greaterThan(limitedVal, maxBound),
                  set(
                    limitedVal,
                    ImageEditor.runSpring(springClock, limitedVal, maxBound, flagWasRunSpring)
                  ),
                  [
            ...
            [
              ...
              cond(and(greaterThan(limitedVal, maxBound), greaterThan(val, prev)), [
                // revert a bit
                set(limitedVal, add(limitedVal, divide(sub(prev, val), 1.2))),
              ]),
              set(prev, val),
                        ...
                        cond(
                          ...
                          cond(
                            ...
                            [
                              ...
                              set(this.opacity, ImageEditor.runTiming(this.gridClock, this.opacity, 1)),
                            ],
                            set(this.opacity, ImageEditor.runTiming(this.gridClock, this.opacity, 0))
                          )
                        ),
                        this.opacity,
                      ])}
