...
class NextjsComponent extends Component {
  ...
  readDefaultBuildManifest(nextConfigPath) {
    return fse.readJSON(join(nextConfigPath, '.serverless/next/default-lambda/manifest.json'))
  }

  async readApiBuildManifest(nextConfigPath) {
    const path = join(nextConfigPath, '.serverless/next/api-lambda/manifest.json')
  ...
  buildDefaultLambda(nextConfigPath, buildManifest) {
    return Promise.all([
      ...
      writeJson(join(nextConfigPath, DEFAULT_LAMBDA_CODE_DIR, 'manifest.json'), buildManifest),
      copy(
  ...
  async buildApiLambda(nextConfigPath, apiBuildManifest) {
    return Promise.all([
      ...
      writeJson(join(nextConfigPath, API_LAMBDA_CODE_DIR, 'manifest.json'), apiBuildManifest),
    ])
  }
  ...
  async deploy(inputs = {}) {
    ...
    const uploadHtmlPages = Object.values(defaultBuildManifest.pages.html).map(page =>
      bucket.upload({
