...
export default config => {
  ...
  } else if (config.noindex || defaults.noindex || config.dangerouslySetAllPagesToNoIndex) {
  ...
  if (config.hasOwnProperty('openGraph')) {
    ...
    if (config.openGraph.images && config.openGraph.images.length) {
      config.openGraph.images.forEach((image, index) => {
        ...
        if (image.width) {
          tagsToRender.push(
            <meta key={`og:image:width0${index}`} property="og:image:width" content={image.width} />
          )
  ...
  const cleanHref = href.indexOf('?') > 0 ? href.substring(0, href.indexOf('?')) : href

  tagsToRender.push(<link rel="canonical" href={cleanHref} key="canonical" />)
  tagsToRender.push(<link rel="alternate" href={cleanHref} hrefLang="x-default" key="alternate" />)
  tagsToRender.push(
