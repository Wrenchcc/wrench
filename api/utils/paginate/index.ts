...
const removeNodesBeforeAndIncluding = (
  ...
  return nodesAccessor
    .clone()
    .where(orderColumn, comparator, columnValue)
    .whereNot({ id })
}
...
const removeNodesAfterAndIncluding = (
  nodesAccessor,
  cursorOfInitialNode,
  { orderColumn, ascOrDesc }
) => {
...
const getNodesLength = async nodesAccessor => nodesAccessor.length
...
//   node
// }
const convertNodesToEdges = (nodes, _, { orderColumn }) => nodes.map(node => ({
